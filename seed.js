...
mongoose.connect(process.dbURL, 
...
const deleteAllActivity = async () => {
    try {
        await activityClass.deleteMany({})
        console.log("All Activity have been removed")
    } catch (err) {
        console.log(err)
    } 
}
...
const activitySeed = [
    {
        ...
        additional_information: `Anak proses kartilago dan sendi torak mengakibatkan gerakan bernapas terganggu dan kemampuan perenggangan torak berkurang. runtuh `,
        registration_deadline: new Date("2020-06-12T17:00:00.000Z"),
        schedule: {
            start_date: new Date("Sat Jun 13 2020 00:00:00 GMT+0700 (WIB)"),
            end_date: new Date("Tue May 12 2020 13:00:00 GMT+0700 (WIB)")
...
const seedActivity = (async () => {
    ...
    activitySeed.forEach(activity => {
        activityClass.create(activity, (err, activity) => {
            if (err) {
